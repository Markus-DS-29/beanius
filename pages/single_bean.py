...
def fetch_single_beans_info_from_db(index_id):
    ...
    cursor.execute('SELECT title, source_url, rating_value, review_count FROM beans_info WHERE `index` = %s', (index_id,))
...
if __name__ == "__main__":
    single_product_index_id = st.number_input("Enter the product index ID", min_value=1, step=1)
