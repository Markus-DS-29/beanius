...
# Initialize HuggingFace model endpoint
...
def start_recording():
    ...
    # Automatically process and transcribe the audio
...
def process_and_transcribe():
    ...
    preprocess_audio(wav_filename, processed_file_path)
...
with col2:
    if st.button('Stop Recording') and st.session_state.recording:
        st.session_state.recording = False
        status.text('Recording stopped. Processing and transcribing...')
